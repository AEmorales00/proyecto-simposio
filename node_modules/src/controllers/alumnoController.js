const Alumno = require('../models/alumnoModel');

exports.getAllAlumnos = (req, res) => {
    const connection = req.app.get('connection'); // Obtén la conexión desde app.js
    const alumno = new Alumno(connection); // Pasa la conexión al modelo

    alumno.getAll((err, results) => {
        if (err) {
            console.error(err);
            res.status(500).send('Error al obtener los alumnos');
        } else {
            res.json(results);
        }
    });
};

exports.createAlumno = (req, res) => {
    const nuevoAlumno = req.body;
    const connection = req.app.get('connection'); // Obtén la conexión desde app.js
    const alumno = new Alumno(connection); // Pasa la conexión al modelo

    alumno.create(nuevoAlumno, (err, results) => {
        if (err) {
            console.error('Error en la base de datos:', err); // Imprime el error en la consola
            res.status(500).json({ error: 'Hubo un error al registrar el alumno' });
        } else {
            res.status(201).json({ message: 'Alumno registrado exitosamente', data: results });
        }
    });
};

exports.getEstadisticasEdad = (req, res) => {
    const connection = req.app.get('connection'); // Obtén la conexión desde app.js
    const alumno = new Alumno(connection); // Pasa la conexión al modelo

    alumno.getEstadisticasEdad((err, results) => {
        if (err) {
            console.error('Error al obtener estadísticas:', err);
            res.status(500).json({ error: 'Hubo un error al obtener las estadísticas' });
        } else {
            res.json(results); // Devuelve las estadísticas en formato JSON
        }
    });
};

// Asegúrate de exportar las funciones
module.exports = exports;